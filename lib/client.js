window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    const inject = ['slots', 'connection', 'layout', 'uiSession', 'locale']

    // ---- Localization ---------------------------------------------------------
    // The plugin follows the dsh language setting. The Host serves one catalog
    // keyed by the Chinese source text (src/messages.js); the client renders the
    // dictionary of the active locale, so a missing entry falls back to the
    // source string instead of an empty label. Keys may carry {name} holes: a
    // rendered string such as "已选 3/3 篇" is then matched against the template
    // "已选 {selected}/{total} 篇".
    let messages = { en: {} }
    let patterns = []
    let activeLocale = 'en'
    ...
    function compilePatterns(dict) {
      ...
      for (const [key, value] of Object.entries(dict || {})) {
        ...
        const names = []
        let source = ''
        for (const part of key.split(/(\{[A-Za-z0-9_]+\})/u)) {
          const hole = /^\{([A-Za-z0-9_]+)\}$/u.exec(part)
          if (hole) { names.push(hole[1]); source += '([\\s\\S]+?)' }
          else source += part.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')
        }
        patterns.push({ regex: new RegExp(`^${source}$`, 'u'), names, template: value })
      }
    }
    function fill(template, values) {
      if (!values) return template
      return template.replace(/\{([A-Za-z0-9_]+)\}/gu,
    ...
    function translate(text, values) {
      ...
      if (!dict) return text
      if (values) return fill(dict[text] ?? text, values)
      ...
      for (const entry of patterns) {
        ...
        entry.names.forEach((name, index) => {
          ...
          // 插值片段优先按词典直译（例如"正在解析"），否则保留原样。
          found[name] = dict[raw] ?? raw
        })
        return fill(entry.template, found)
      }
      return text
    }
    function bumpLocale() {
    ...
    function installMessages(next) {
      if (!next || typeof next !== 'object') return
      messages = next.zh ? { zh: { ...next.zh }, en: { ...(next.en || {}) } }
        : { en: { ...(next.en || {}) } }
      compilePatterns(messages[activeLocale] || {})
      bumpLocale()
    }
    function setActiveLocale(id) {
      const next = normalizeLocale(id)
      if (next === activeLocale && patterns.length) return
      activeLocale = next
      compilePatterns(messages[activeLocale] || {})
      bumpLocale()
    }
    function useLocaleRevision() {
      ...
      return React.useSyncExternalStore(subscribe, () => localeRevision)
    }
    // Text props translated alongside string children (labels live in children).
    const TEXT_PROPS = ['placeholder', 'title', 'aria-label']
    function createElement(type, props, ...children) {
      let next = props
      if (props) for (const name of TEXT_PROPS) {
      ...
      return React.createElement(type, next, ...children.map(child =>
    ...
    function StudioView({ sessionId, onExit }) {
      ...
      React.useEffect(() => {
        ...
        // 会话建立后再取一次词典：启动时的无会话调用可能被连接层拒绝。
        void rpc('getMessages').then(value => installMessages(value)).catch(() => {})
      ...
      function stateLabel(state) {
        return translate(({ running: '进行中', succeeded: '完成', failed: '失败', interrupted: '中断' })[state] || state)
      }
      function jobKindLabel(kind) {
        return translate(({ import: '导入文献', outline: '生成大纲', report: '生成报告', deck: '生成演示',
      ...
      function jobDuration(job) {
        ...
        return seconds >= 60
          ? translate('{minutes} 分 {seconds} 秒', { minutes: Math.floor(seconds / 60), seconds: seconds % 60 })
          : translate('{seconds} 秒', { seconds })
      }
      ...
      return h('section', { className: 'dls', 'aria-label': 'NotebookStudio 工作台' },
        h('header', { className: 'dls-head' },
          h('div', { className: 'dls-brand' },
            ...
            h('div', { className: 'dls-project' }, h('h1', null, 'Notebook Studio'),
              h('p', { className: 'dls-muted', title: snapshot?.workspacePath || '' },
                snapshot?.workspaceName
                  ? translate('工作区 · {name}', { name: snapshot.workspaceName })
                  : '正在加载文献项目…'))),
          h('div', { className: 'dls-actions' },
        ...
        h('div', { className: 'dls-strip' },
          ...
          h('div', { className: 'dls-status' + (warning ? ' warning' : latestJob?.state === 'failed' ? ' failed' : ''), role: 'status' },
            latestJob ? stateLabel(latestJob.state) + ' · ' + translate(latestJob.error || latestJob.phase) : '上传文献，开始创作')),
            ...
            h('details', null, h('summary', null, '任务记录' + (jobs.length ? '（' + jobs.length + '）' : '')),
              h('div', { className: 'dls-history' }, jobs.map(job => h('div', { className: 'dls-source', key: job.id },
                ...
                h('p', { className: job.error ? 'dls-error' : 'dls-muted' },
                  translate(job.error || stateLabel(job.state))
                  + (job.state !== 'running' && job.phase && job.phase !== stateLabel(job.state)
                    ? ' · ' + translate(job.phase) : '')
                  + (jobDuration(job) ? ' · ' + translate('耗时 {duration}', { duration: jobDuration(job) }) : '')))))),
    ...
    function apply(ctx) {
      ...
      // 语言跟随 dsh 设置：优先宿主 locale 服务，缺失时退回浏览器语言。
      if (ctx.locale?.getLocale) {
        setActiveLocale(ctx.locale.getLocale().active)
        ctx.effect(() => ctx.locale.subscribe(() => setActiveLocale(ctx.locale.getLocale().active)),
          'Notebook Studio 语言')
      } else if (typeof navigator !== 'undefined') {
        setActiveLocale(navigator.language)
      }
      // 词典与会话无关，进入工作台前取用；取不到时界面继续显示中文原文。
      void ctx.connection?.rpc?.call?.('/studio', 'getMessages', {})
        .then(result => { if (result?.ok) installMessages(result.value) })
        .catch(() => {})
